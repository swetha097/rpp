...
DEFAULT_SRC_FOLDER_1="/media/RESIZED_IMAGES_300x300_bs_128/"
DEFAULT_SRC_FOLDER_2="/media/RESIZED_IMAGES_300x300_bs_128/"
...
if [[ "$1" -lt 0 ]] | [[ "$1" -gt 82 ]]; then
    echo "The starting case# must be in the 0-82 range!"
    echo
    ...
    echo "S     CASE_START (Starting case# (0-82))"
    echo "E     CASE_END (Ending case# (0-82))"
    exit 1
fi

if [[ "$2" -lt 0 ]] | [[ "$2" -gt 82 ]]; then
    echo "The ending case# must be in the 0-82 range!"
    echo
    echo "The rawLogsGenScript.sh bash script runs the RPP performance testsuite for AMDRPP functionalities in HOST/OCL/HIP backends."
    echo
    echo "Syntax: ./rawLogsGenScript.sh <S> <E>"
...
printf "\n\nUsage: ./BatchPD_host_pkd3 <src1 folder> <src2 folder (place same as src1 folder for single image functionalities)> <u8 = 0 / f16 = 1 / f32 = 2 / u8->f16 = 3 / u8->f32 = 4 / i8 = 5 / u8->i8 = 6> <outputFormatToggle (pkd->pkd = 0 / pkd->pln = 1)> <case number = 0:82> <verbosity = 0/1>"
...
printf "\n\nUsage: ./BatchPD_host_pln1 <src1 folder> <src2 folder (place same as src1 folder for single image functionalities)> <u8 = 0 / f16 = 1 / f32 = 2 / u8->f16 = 3 / u8->f32 = 4 / i8 = 5 / u8->i8 = 6> <outputFormatToggle (pkd->pkd = 0 / pkd->pln = 1)> <case number = 0:82> <verbosity = 0/1>"
...
printf "\n\nUsage: ./BatchPD_host_pln3 <src1 folder> <src2 folder (place same as src1 folder for single image functionalities)> <u8 = 0 / f16 = 1 / f32 = 2 / u8->f16 = 3 / u8->f32 = 4 / i8 = 5 / u8->i8 = 6> <outputFormatToggle (pkd->pkd = 0 / pkd->pln = 1)> <case number = 0:82> <verbosity = 0/1>"
